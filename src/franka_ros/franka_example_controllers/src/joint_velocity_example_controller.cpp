...
//   q_dot = jacobian_pinv * (xd_dot + K * error); 
...
//   // ROS_INFO_STREAM("q_dot: " << q_dot.transpose());
// }
...
// void JointVelocityExampleController::publisherFunction() {
  
//   geometry_msgs::PoseStamped pose_msg;
//   pose_msg.header.stamp = ros::Time::now();  
//   pose_msg.header.frame_id = "world"; 
  
//   pose_msg.pose.position.x = position_d_target_[0];
//   pose_msg.pose.position.y = position_d_target_[1];
//   pose_msg.pose.position.z = position_d_target_[2];
//   pose_msg.pose.orientation.x = orientation_d_target_.x();
//   pose_msg.pose.orientation.y = orientation_d_target_.y();
//   pose_msg.pose.orientation.z = orientation_d_target_.z();
//   pose_msg.pose.orientation.w = orientation_d_target_.w();
  
//   target_publisher.publish(pose_msg);
// }
...
Eigen::Matrix<double, 3, 1> computePoseError(const Eigen::Affine3d& T, const Eigen::Affine3d& Td) {
  ...
  // Calcolo dell'errore di rotazione usando l'angolo-asse
...
void JointVelocityExampleController::update(const ros::Time& time, const ros::Duration& period) {
  ...
  // double scale = 0.05; 
  // double omega = 0.2;  
  // Eigen::Vector3d center(0.5, 0.0, 0.5);

  // elapsed_time_ += period;
  // double theta =omega*elapsed_time_.toSec();

  // position_d_target_[0]= center[0]+scale*std::sin(theta )+2*scale*std::sin(2*theta ); 
  // position_d_target_[1]= center[1];  
  // position_d_target_[2]= center[2]+scale*std::cos(theta )-2*scale*std::cos(2*theta ); 
